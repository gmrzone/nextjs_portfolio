...
**Single-page site.** `pages/index.tsx` is the only real page; it renders every section (`hero`, `skills`, `experience`, `projects`, `about`, `contact`) in order inside `MainLayout`. Each section lives in `components/home/<section>/index.tsx`. There is no routing to speak of — navigation is in-page scrolling.
...
**Newer sections drive motion from React state, not `classList`.** The Experience section and the tilt-on-hover cards (awards, contact channels) keep their animation state in React and hand-roll the motion in [hooks/useCardTilt.ts](hooks/useCardTilt.ts) and [components/home/experience/motion.ts](components/home/experience/motion.ts) — `useScrollProgress` (a scroll-linked beam), `useActivated`, `useCountUp`, `useCardTilt`. Two rules hold throughout: pointer/scroll handlers write straight to the node and coalesce with `requestAnimationFrame` rather than calling `setState` per event, and every one of them honours `prefers-reduced-motion`. Prefer this over the older `classList` pattern in new work — it also keeps class strings literal in JSX, so Tailwind cannot purge them.
...
**All content is hardcoded in [components/home/data.ts](components/home/data.ts).** `skillsData`, `experienceData`, `awardData`, `ProjectData` plus their interfaces, with images imported statically from `public/` as `StaticImageData`. Editing portfolio content (projects, skills, experience, awards) means editing this one file, not a CMS or fetch. Icons are Font Awesome **Pro** class strings (`fad fa-code`, `fal fa-phone-alt`) loaded from a CDN `<link>` in `_document.tsx` — icons render as blanks if that CDN is unreachable.

### Three styling systems coexist

1. **Tailwind utilities** — the default for layout and color.
2. **`*.module.scss`** in `styles/` — reserved for the two components with complex multi-stage transitions (`customInput`, `themeSwitcher`), driven by CSS custom properties defined in `globals.css` (e.g. `--input-transition-in-*-delay`).
