...
const ButtonLink: NextPage<ButtonProps> = ({ text, action, icon = null, forceBig, cssClasses = null, ...attr }) => {
    return (
        <a
            ...
            } inline-block self-center rounded-md flex-nowrap transition-colors duration-300 text-bg-sec font-semibold cursor-pointer desktop-st:text-lg desktop-st:px-4 desktop-st:py-3 ${
                icon && "space-x-2"
            } ${
                action
                    ? "bg-action hover:bg-action-hover dark:bg-blue-600 dark:text-bg-sec-inverted dark:hover:bg-blue-900"
                    : "bg-sec dark:bg-bg-sec-dark dark:hover:bg-bg-sec-inverted hover:bg-blue-700 dark:text-main"
