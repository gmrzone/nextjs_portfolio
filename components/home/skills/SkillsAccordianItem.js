...
const DesktopFullSkillModalItem = dynamic(
    () => import('./DesktopFullSkillModalItem'),
    { ssr: false }
  )
const SkillsAccordianItem = ({item: {title, meta, data, icon, id,}, last ,  activeAccordian , activateAccordian}) => {
    ...
    return (
        <div className={`w-full max-w-full desktop-st:max-w-md space-y-2 accordian-item`}>
            ...
            <div className="bg-sec text-white px-3 font-bold mb-8 py-3 text-center shadow-md rounded-md hidden cursor-pointer transition-colors duration-300 bg-opacity-100 hover:bg-opacity-75 desktop-st:block" onClick={openDesktopSkillModal}>
                View All
            </div>
            <style jsx>{`
